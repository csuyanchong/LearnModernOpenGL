...
void Mesh::setUp() {
  // 创建OpenGL缓存
  createAndPopulateBuffer(vao, vbo, ibo, vertices, indices);
}

void Mesh::createAndPopulateBuffer(GLuint& vao, GLuint& vbo, GLuint& ibo, std::vector<Vertex> _vertices, std::vector<GLuint> _indices) {
  vao = 0;
  vbo = 0;
  ibo = 0;
  // 创建vao
